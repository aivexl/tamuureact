// Database cleanup script - CommonJS version
...
async function cleanup() {
    ...
    const { data: remainingTemplates } = await supabase
        .from('templates')
        .select('id, name, slug');
    console.log('   TEMPLATES:', remainingTemplates ? remainingTemplates.map(t => t.slug + ' (' + t.name + ')') : 'None');

    const { data: remainingInvitations } = await supabase
        .from('invitations')
        .select('id, name, slug');
    console.log('   INVITATIONS:', remainingInvitations ? remainingInvitations.map(i => i.slug + ' (' + i.name + ')') : 'None');
