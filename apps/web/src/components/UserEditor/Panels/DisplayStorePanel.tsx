...
export const DisplayStorePanel: React.FC<DisplayStorePanelProps> = ({
    ...
    // Fetch display templates from templates API (filtered by type='display')
    useEffect(() => {
        const fetchDisplayTemplates = async () => {
            setIsLoading(true);
            try {
                // Fetch all templates, then filter by type='display'
                ...
                console.log('[DisplayStore] Loaded display templates:', displayTemplates.length);
            } catch (error) {
                console.error('[DisplayStore] Failed to fetch display templates:', error);
                setDisplays([]);
            } finally {
                setIsLoading(false);
            }
    ...
    const handleApplyDisplay = async () => {
        ...
        try {
            ...
            // Update the invitation with the display template's sections
            // This creates a "display design" linked to the invitation
            await invitationsApi.update(currentInvitationId, {
                display_template_id: selectedDisplay.id,
                display_sections: templateData.sections || [],
            });
