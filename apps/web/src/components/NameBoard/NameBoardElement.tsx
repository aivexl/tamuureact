...
export const NameBoardElement: React.FC<NameBoardElementProps> = ({ layer, isEditor, onContentLoad }) => {
    ...
    // DEBUG: Monitor guest resolution state
    useEffect(() => {
        if (!isEditor) {
            console.log('[NameBoard] Context Sync Check:', {
                hasGuestData: !!guestData,
                guestName: guestData?.name,
                layerType: layer.type
            });
        }
    }, [guestData, isEditor, layer.type]);
    ...
    if (isEditor) {
        ...
        displayedName = greetingName || '';
        displayedTier = greetingTier;
    } else {
        // NAME BOARD (INVITATION): Prioritize identity from store
    ...
    const getBackgroundStyle = (): React.CSSProperties => {
        if (isPureTransparent) return { background: 'transparent' };
        if (config.gradientEnabled) return { background: `linear-gradient(135deg, ${config.gradientStart}, ${config.gradientEnd})` };
        const bgColor = config.backgroundColor || variant.bgColor;
        if (bgColor.startsWith('linear-gradient')) return { background: bgColor };
    ...
    const getTextStyle = (): React.CSSProperties => {
        ...
        if (!isPureTransparent && variant.category === 'neon') {
            const glowColor = String(baseStyle.color);
            baseStyle.textShadow = `0 0 10px ${glowColor}, 0 0 20px ${glowColor}, 0 0 40px ${glowColor}40`;
        }
    ...
    return (
        <m.div
            ...
            style={{
                ...
                border: isPureTransparent ? 'none' : `${config.borderWidth}px solid ${config.borderColor || variant.borderColor}`,
                backdropFilter: variant.category === 'glass' ? 'blur(12px)' : 'none'
            }}
        >
