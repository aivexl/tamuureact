...
export const AdminShopSettings: React.FC = () => {
    ...
    const [currentTab, setCurrentTab] = useState<'carousel' | 'ads' | 'placement' | 'config' | 'categories'>('carousel');
        ...
        <div className="max-w-7xl mx-auto space-y-8 pb-32">
            ...
            <div className="flex gap-2 p-1 bg-white/5 border border-white/10 rounded-2xl w-fit">
                ...
                <button 
                    ...
                    className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${currentTab === 'categories' ? 'bg-[#FFBF00] text-[#0A1128]' : 'text-slate-400 hover:text-white'}`}
                >
                    Kategori Produk
                </button>
                <button 
...
import { 
    ...
    UploadCloud, Search, Star, Sparkles, ShoppingBag, Store,
    Filter, ArrowRight, ExternalLink, ShieldCheck, Phone, MessageSquare,
    Globe, Tag, LayoutGrid, Utensils, Camera, Palette, Building2, Music, Gift, Heart
} from 'lucide-react';
import { toast } from 'react-hot-toast';
import { PremiumLoader } from '../ui/PremiumLoader';
import { admin, shop, storage, safeFetch, API_BASE, shopCategories, type ShopCategory } from '../../lib/api';
import { useStore } from '../../store/useStore';
import { useAdminProducts, useAdminUpdateProduct, useAdminUpdateVendor, useAdminVendors } from '../../hooks/queries/useShop';
                        ...
                        <div className="p-8 space-y-8">
                            ...
                            <div className="bg-white/[0.02] border border-white/5 rounded-3xl p-6">
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest ml-1">Icon (Lucide)</label>
                                        <select 
                                            value={newCategory.icon}
                                            onChange={e => setNewCategory({ ...newCategory, icon: e.target.value })}
                                            className="w-full bg-[#0A0A0A] border border-white/10 rounded-xl px-4 py-3 text-white text-xs font-bold appearance-none"
                                        >
                                            <option value="LayoutGrid">Default (Grid)</option>
                                            <option value="Sparkles">Sparkles</option>
                                            <option value="Heart">Heart</option>
                                            <option value="Utensils">Catering / Food</option>
                                            <option value="Camera">Photography</option>
                                            <option value="Palette">Decoration</option>
                                            <option value="Building2">Venue</option>
                                ...
                                <table className="w-full text-left">
                                    <thead>
                                        <tr className="border-b border-white/5">
                                            ...
                                            <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Slug Identifier</th>
                                    ...
                                    <tbody className="divide-y divide-white/5">
                                        {categories.map((cat) => (
                                            <tr key={cat.id} className="group hover:bg-white/[0.01] transition-colors">
                                                ...
                                                <td className="px-6 py-5">
                                                    <span className="text-[10px] font-mono text-slate-500 uppercase">{cat.slug}</span>
                                                </td>
...
// Simple Icon Resolver for Admin UI
const IconResolver = ({ name }: { name: string }) => {
    switch (name) {
...
... User modified the `new_string` content to be: import React, { useState, useEffect, useRef } from 'react';
import { m, AnimatePresence } from 'framer-motion';
import { 
    Plus, Trash2, GripVertical, Save, Image, Link as LinkIcon, 
    AlertCircle, Megaphone, Check, X, LayoutTemplate, 
