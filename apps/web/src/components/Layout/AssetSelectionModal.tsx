        ...
        >
            ...
            <div className="p-4 max-h-[60vh] overflow-y-auto premium-scroll">
                ...
                {type === 'social_mockup' && (
                    <div className="space-y-3">
                        {[
                            { id: 'instagram', label: 'Instagram', icon: <Instagram size={18} className="text-[#E4405F]" />, handle: '@instagram' },
                            { id: 'x', label: 'X (Twitter)', icon: <XLogo size={16} className="text-white" />, handle: '@x_app' },
                            { id: 'whatsapp', label: 'WhatsApp', icon: <ExternalLink size={18} className="text-[#25D366]" />, handle: '0812345678' },
                            { id: 'tiktok', label: 'TikTok', icon: <Music size={18} className="text-white" />, handle: '@tiktok' },
                        ].map((p) => (
                            <button
                                key={p.id}
                                onClick={() => onSelect({ socialMockupConfig: { platform: p.id === 'x' ? 'twitter' : p.id as any } })}
                                className="w-full group flex items-center justify-between p-4 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/5 hover:border-white/20 transition-all duration-300"
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex flex-col text-left">
                                        <span className="text-[11px] font-black text-white/90 uppercase tracking-widest">{p.label}</span>
                                        <span className="text-[10px] text-white/40 font-medium">{p.handle}</span>
                                    </div>
                                </div>
